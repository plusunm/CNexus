"""L3 scheduler v5 — global cluster + CRDT (signal only)."""
...
class L3SchedulerV5:
    ...
    def __post_init__(self) -> None:
        ...
        if self.trace_base_dir:
            get_deterministic_log().configure_persistence(self.trace_base_dir)
        if cluster.consensus.leader is None:
    ...
    def enqueue_signal(
        ...
        *,
        ...
        get_global_cluster_runtime().submit(
            {
                "id": uuid.uuid4().hex,
                "type": "l3.task",
                "label": label,
    ...
    def status_payload(self) -> Dict[str, Any]:
        ...
        cluster = get_global_cluster_runtime()
        return {
            "scheduler": "l3-global-cluster-v5",
            "ticks": self._ticks,
    ...
    def is_idle(self) -> bool:
        return self.queue_length() == 0 and get_global_cluster_runtime().cluster_health()
